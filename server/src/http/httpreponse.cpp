...
// Construct the response message into the buffer according to the response code
void HttpResponse::MakeResponse(Buffer& buff) {
    if (stat((srcDir_ + path_).data(), &mmFileStat_) < 0 || S_ISDIR(mmFileStat_.st_mode)) {
        // If the file is not found, return 404
        code_ = 404;
    } else if (!(mmFileStat_.st_mode & S_IROTH)) {
        // If the file is forbidden to read for others, return 403
        code_ = 403;
    } else if (code_ == -1) {
        code_ = 200;
    }
...
// If the response code is 400, 403, or 404, set the path to the corresponding error page
...
// Get the file type according to the suffix of the file
string HttpResponse::GetFileType_() {
    string::size_type idx = path_.find_last_of('.');
    // If the suffix is not found, return text/plain
    if (idx == string::npos) {
        return "text/plain";
    }
...
// Construct the HTML error page into the buffer
